...
if ($clientsdetails["companyname"]) {
    ...
    if(!$clientsdetails["tax_id"]){ $pdf->SetX(15); $pdf->Cell(90, 4, Lang::trans('invoicesattn') . ': ' . $clientsdetails["firstname"] . ' ' . $clientsdetails["lastname"], 0, 1, 'L'); }
} else { $pdf->SetX(15); $pdf->Cell(90, 4, $clientsdetails["firstname"] . " " . $clientsdetails["lastname"], 0, 1, 'L'); }
...
$pdf->SetX(15); $pdf->Cell(90, 4, $clientsdetails["city"] . ", " . $clientsdetails["state"] . ", " . $clientsdetails["postcode"], 0, 1, 'L');
$pdf->SetX(15); $pdf->Cell(90, 4, $clientsdetails["country"], 0, 1, 'L');
$pdf->Ln(2);
if (array_key_exists('tax_id', $clientsdetails) && $clientsdetails['tax_id']) {
    $pdf->SetX(15); $pdf->Cell(90, 4, $taxIdLabel . ': ' . $clientsdetails['tax_id'], 0, 1, 'L');
    $pdf->SetX(15); $pdf->Cell(90, 4, 'Place of supply: ' . substr($clientsdetails['tax_id'], 0, 2) . ' - ' . $clientsdetails["state"], 0, 1, 'L');
}
if ($customfields) {
    foreach ($customfields as $customfield) { $pdf->SetX(15); $pdf->Cell(90, 4, $customfield['fieldname'] . ': ' . $customfield['value'], 0, 1, 'L'); }
}
$endY = $pdf->GetY();
...
if($status != 'Paid'){ $pdf->SetX(120); $pdf->Cell(75, 6, Lang::trans('invoicesdatedue') . ': ' . $duedate, 0, 1, 'L', 1); }
$pdf->SetXY(15, $endY);
$pdf->Ln(5);
...
if($clientsdetails["country"] == "India"){
    ...
    foreach ($invoiceitems as $item) {
        if($item['description'] != 'Round Off'){
            ...
            // 3. Fallback: Default WHMCS Types
            if (!$ruleMatched) {
                if ($itemType == 'Hosting' || $itemType == 'PromoHosting') {
                    $sacCode = $addonConfig['sac_hosting'] ?? '998315';
                } 
                elseif ($itemType == 'Upgrade') {
                    $sacCode = $addonConfig['sac_upgrade'] ?? '998315';
                }
                elseif (strpos($itemType, 'Domain') !== false) {
                    $sacCode = $addonConfig['sac_domain'] ?? '998319';
                }
                elseif ($itemType == 'Addon') {
                    $sacCode = $addonConfig['sac_addon'] ?? '998313';
                }
                elseif ($itemType == 'Setup') {
                    $sacCode = $addonConfig['sac_setup'] ?? '998313';
                }
                elseif ($itemType == 'LateFee') {
                    // Specific Late Fee check as requested
                    $sacCode = $addonConfig['sac_latefee'] ?? '998313';
                    $displayType = 'Late Fee';
                }
                // Note: "Item" (Billable Items) is intentionally left out to fall to $sac_default or be caught by Rules.
            }
    ...
    if (isset($round_off) && $round_off != 0) $tblhtml .= '<td align="center">₹' . number_format(preg_replace("/[^0-9\.]/", '', $subtotal) - $round_off,2,".",",") . '</td>';
    ...
    $tblhtml .= '<td align="center">₹'.number_format(preg_replace("/[^0-9\.]/", '', $tax) + preg_replace("/[^0-9\.]/", '', $tax2),2,".",",").'</td></tr>';

    if (isset($round_off) && $round_off != null) $tblhtml .='<tr bgcolor="#efefef" height="30" style="font-weight:bold;"><td align="right">Round Off</td><td colspan="2">&nbsp;</td><td align="center">₹'.$round_off.'</td></tr>';

    $tblhtml .='<tr bgcolor="#efefef" height="30" style="font-weight:bold;"><td align="right">Total Amount Incl. GST</td><td colspan="2">&nbsp;</td>';
    if ($credit != 0) $tblhtml .= '<td align="center">₹'.number_format(preg_replace("/[^0-9\.]/", '', $total) + preg_replace("/[^0-9\.]/", '', $credit),2,".",",").'</td>';
    ...
    // Non-India
    $tblhtml = '<table width="100%" bgcolor="#ccc" cellspacing="1" cellpadding="2" border="0"><tr height="30" bgcolor="#efefef" style="font-weight:bold;text-align:center;"><td width="80%">' . Lang::trans('invoicesdescription') . '</td><td width="20%">' . Lang::trans('quotelinetotal') . ' (' . $currencycode . ')</td></tr>';
    foreach ($invoiceitems as $item) { $tblhtml .= '<tr bgcolor="#fff"><td align="left">'.nl2br($item['description']).'<br /></td><td align="center">'.$item['amount'].'</td></tr>'; }
    $tblhtml .= '<tr height="30" bgcolor="#efefef" style="font-weight:bold;"><td align="right">' . Lang::trans('invoicessubtotal') . '</td><td align="center">' . $subtotal . '</td></tr>';
    if ($taxname) $tblhtml .= '<tr height="30" bgcolor="#efefef" style="font-weight:bold;"><td align="right">'.$taxrate.'% '.$taxname.'</td><td align="center">'.$tax.'</td></tr>';
    if ($taxname2) $tblhtml .= '<tr height="30" bgcolor="#efefef" style="font-weight:bold;"><td align="right">'.$taxrate2.'% '.$taxname2.'</td><td align="center">'.$tax2.'</td></tr>';
    $tblhtml .= '<tr height="30" bgcolor="#efefef" style="font-weight:bold;"><td align="right">'.Lang::trans('invoicescredit').'</td><td align="center">'.$credit.'</td></tr>';
    $tblhtml .= '<tr height="30" bgcolor="#efefef" style="font-weight:bold;"><td align="right">'.Lang::trans('invoicestotal').'</td><td align="center">'.$total.'</td></tr></table>';
}
